...
sudo apt update
sudo apt install -y playerctl pulseaudio-utils
...
firefox_audio_playing() {
    [[ "$BROWSER_NAME" == "Firefox" ]] || return 1
    command -v pactl >/dev/null 2>&1 || return 1

    # Firefox may fail to create an MPRIS player even though its audio stream
    # is actively playing. PulseAudio/PipeWire exposes such a stream as
    # "Corked: no". Match either the process binary or application name.
    pactl list sink-inputs 2>/dev/null | awk '
        ...
        {
            firefox = (
                $0 ~ /application\.process\.binary = "firefox"/ ||
                $0 ~ /application\.name = "Firefox"/ ||
                $0 ~ /application\.process\.binary = "firefox-bin"/
            )
            active = ($0 ~ /Corked: no/)
            if (firefox && active) {
                found=1
...
while true; do
    ...
    # ------------------------------------------------------------------
    # Firefox fallback: no MPRIS player, but Firefox has active audio.
    # ------------------------------------------------------------------
    if [[ -z "$PLAYER" ]] && firefox_audio_playing; then
        rm -f "$NO_PLAYER_WARNING"

        TITLE="Firefox video (MPRIS unavailable)"
        TITLE_HASH="$(printf '%s' "firefox-audio|$TITLE" | sha256sum | awk '{print $1}')"
        ...
        if [[ -f "$LAST_STATE" ]]; then
            ...
            LAST_SOURCE="$(grep '^source=' "$LAST_STATE" 2>/dev/null | cut -d= -f2- || true)"
        fi

        if [[ "$LAST_SOURCE" == "firefox-audio" &&
        ...
        {
            echo "source=firefox-audio"
            echo "title_hash=$TITLE_HASH"
            echo "last_seen=$NOW"
            echo "position="
            echo "status=AudioActive"
        } > "$LAST_STATE"
    ...
    # No MPRIS player and no active Firefox audio: nothing is playing.
    if [[ -z "$PLAYER" ]]; then
        rm -f "$LAST_STATE"

        if ! list_selected_players | grep -q .; then
            if [[ ! -f "$NO_PLAYER_WARNING" ]]; then
                echo "INFO: No MPRIS player is visible for $BROWSER_NAME."
                if [[ "$BROWSER_NAME" == "Firefox" ]]; then
                    echo "Firefox audio fallback will be used when an active audio stream appears."
                fi
    ...
    # Primary signal: playback position advances.
    if [[ "$LAST_SOURCE" == "mpris" &&
          "$TITLE_HASH" == "$LAST_HASH" &&
          "$LAST_SEEN" =~ ^[0-9]+$ &&
          "$POS" =~ ^[0-9]+$ &&
          "$LAST_POS" =~ ^[0-9]+$ ]]; then
    ...
    # Secondary signal: browser explicitly reports Playing.
    if (( SHOULD_COUNT == 0 )) &&
       [[ "$LAST_SOURCE" == "mpris" &&
          "$STATUS" == "Playing" &&
          "$TITLE_HASH" == "$LAST_HASH" &&
    ...
    # If Firefox exposes an MPRIS player but both its status and position are
    # unreliable, use active Firefox audio as the final confirmation.
    if (( SHOULD_COUNT == 0 )) &&
       [[ "$BROWSER_NAME" == "Firefox" &&
          "$LAST_SOURCE" == "mpris" &&
          "$TITLE_HASH" == "$LAST_HASH" &&
          "$LAST_SEEN" =~ ^[0-9]+$ ]] &&
       firefox_audio_playing; then
...
echo
echo "Firefox audio-stream fallback:"
if [[ "$BROWSER_NAME" != "Firefox" ]]; then
    echo "Not applicable: selected browser is $BROWSER_NAME"
    ...
    while IFS= read -r line; do
        ...
        pactl list sink-inputs 2>/dev/null | awk '
            BEGIN { RS="Sink Input #"; IGNORECASE=1 }
            {
                firefox = (
                    $0 ~ /application\.process\.binary = "firefox"/ ||
                    $0 ~ /application\.name = "Firefox"/ ||
                ...
                if (firefox) {
                    corked="unknown"
                    if ($0 ~ /Corked: no/) corked="no (ACTIVE)"
                    if ($0 ~ /Corked: yes/) corked="yes (paused)"
                    print "Firefox audio stream found; Corked: " corked
                }
            }
        '
    )
    ...
    if [[ "$FOUND" -eq 0 ]]; then
        ...
        echo "Start a video with audible sound and run this diagnostic again."
    fi
fi

echo
echo "Detection policy:"
echo "1. Advancing MPRIS position"
echo "2. MPRIS Status=Playing"
echo "3. Active Firefox audio stream (PipeWire/PulseAudio fallback)"
echo
echo "Service status:"
